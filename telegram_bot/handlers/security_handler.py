...
from telegram_bot.filters.chat_types import ChatTypeFilter, IsSecurity

security_private_router = Router()

security_private_router.message.filter(ChatTypeFilter(['private']), IsSecurity())


@security_private_router.chat_member(ChatMemberUpdatedFilter(IS_NOT_MEMBER >> IS_MEMBER))
...
@security_private_router.message(CommandStart())
async def on_startup(message: types.Message):
    await message.answer("Приветь охранник!",
